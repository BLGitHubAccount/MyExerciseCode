...
namespace ConsoleApplication1
{
    class Program
    {
        ...
        {
            byte[] arry = new byte[] { 0x00, 0x5a, 0x56, 0x00, 0x01, 0x01, 0x00, 0x00,0x00 };
            //byte result= GetXOR(arry);
           byte[] myresult= CheckAnd(arry);
           byte a= CalculateAdd(arry);
            Console.WriteLine(myresult[8].ToString());
            Console.WriteLine(a.ToString());
            Console.ReadKey();

        }

        public static byte CalculateAdd(byte[] buffer)//取低八位
        {
            int cks = 0;
        ...
        public static byte[] CheckAnd(byte[] sendBytes)
        {
            int sum = 0;
            for (int i = 0; i < sendBytes.Length - 1; i++)
                sum += sendBytes[i]; //将每个数相加
            if (sum > 0xff)
            {
                sum = ~sum;
                sum += 1;
            ...
            sum = sum & 0xff;
            sendBytes[sendBytes.Length - 1] = (byte)sum;
            return sendBytes;
        }
